...
demoquestion<- find_question(questions, "te_numeric_good")
```
...
```{r}
is_text_entry_numeric(demoquestion)
```
...
```{r}
demoquestion<-generate_summary_stats(demoquestion, orientation = "horizontal")
demoquestion[['Table']]
````
