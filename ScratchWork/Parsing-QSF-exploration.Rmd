...
###Revised function to split a single side-by-side question

I want to be able to use map to iterate over the questions. I shouldn't have to make this function include the iteration over the questions; we should be able to do this to questions and then split blocks based on a list of SBS questions.
...
split_side_by_side_q <- function(question) {
  ...
  split_q <- additional_questions %>%
    ...
    map(~list_modify(.x,"QuestionText_SBS" = .x[["QuestionText"]])) %>%
    map(~list_modify(.x,"QuestionText" = str_c(mainq_text,"---",.x[["QuestionText"]]))) %>%
    ...
    map(~ rlist::list.append(.x,qt_notes = if_else("qtNotes" %in% names(mainq),
                                                   list(append(mainq[["qtNotes"]],"This question was split from a side-by-side question.")),
                                                   list("This question was split from a side-by-side question."))))
    
    #Add an additional note that this was split from side-by-side
...
We need to update how we're thinking about user notes. These, too, should be easier to find! They should be labeled with the parent IDs (QIDs) so that we can easily link them to the appropriate quesitons. This has been functionalized above.
