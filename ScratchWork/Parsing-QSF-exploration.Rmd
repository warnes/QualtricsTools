...
questions2 <- questions %>%
  map(~ if(.x[["Qualtrics_qtype"]]=="SBS") {
    append(.,values = split_side_by_side_q(.x))}
    return(.))

    append(., split_side_by_side_q(.x),
           after=match(.x[["PrimaryAttribute"]],names(.)))



  
  map(~if_else(.x[["Qualtrics_qtype"]]=="SBS",~append(.,split_side_by_side_q(.x)),.x))
  
  
  map_if(~.x[["Qualtrics_qtype"]]=="SBS",~split_side_by_side_q(.x))
