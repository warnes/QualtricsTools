...
So lets grab Q15 from the survey - it is MCMA without any recode values.

```{r}
question <- questions[[which(purrr::map_lgl(questions, ~ .x[['Payload']][['DataExportTag']]=="Q15"))]]
```

Now lets generate a Table with this question. The table by default will be *sorted by the N column, then alphabetically by the choices* in the third column.
...
If you would like your table to be in the **choice order**, simply set sort_by to equal "choices_order"

```{r}
question <- mc_multiple_answer_results(question, original_first_rows, sort_by = "choices_order")
...
```{r}
question <- questions[[which(purrr::map_lgl(questions, ~ .x[['Payload']][['DataExportTag']]=="Q13"))]]
```
